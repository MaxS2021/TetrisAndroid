...
class AppModel {
    ...
    fun genefateField(action: String) {
        if (isGameActive()) {
            resetField()
    ...
    private fun resetField(ephemiralCellsOnly: Boolean = true) {
        for (i in 0 until  FieldConstants.ROW_COUNT.value) {
            (0 until FieldConstants.COLUMN_COUNT.value).filter {!ephemiralCellsOnly || field[i][it] ==
            CellConstants.EPHEMERAL.value}.forEach{field[i][it] = CellConstants.EMPTY.value}
        }
    }
    ...
    private fun blockAdditionPosisible(): Boolean {
    ...
    private fun blockAddiionalPossible(): Boolean {
        if (!moveValid(currentBlock?.position as Point, currentBlock?.frameNumber)) {
            return false
        }
        return true
